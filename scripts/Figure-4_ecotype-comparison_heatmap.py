...
# ---------------------------
BASE_DATA = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/3_cotype_impact_assessment'))
...
calibration_code_list = [
    'cultivar_AZWH18', 'cultivar_CI0001',
    'cultivar_DEFAULT', 'cultivar_subset_c'
]
...
if __name__ == '__main__':
    ...
    # Update column name for data subset C
    nrmse_filtered_ordered.rename(columns={'C': 'subset C'}, inplace=True)
    mpe_filtered_ordered.rename(columns={'C': 'subset C'}, inplace=True)
    r2_filtered_ordered.rename(columns={'C': 'subset C'}, inplace=True)
    gain_filtered_ordered.rename(columns={'C': 'subset C'}, inplace=True)
    ...
    create_heatmap_plot(
        ...
        figure_name='Figure-4_ecotype-comparison',
        treatment=treatment,
        XTICK_ROTATION=45,
        y_x_axis_title=-0.01,
